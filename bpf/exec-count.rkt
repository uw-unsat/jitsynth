...
; Instead, merge this functionality with bpf/op.rkt
...
(define (interpret
          instructions curr-state base-pc depth max-depth
          #:allow-bele? [allow-bele? #f]
          #:outfile [outfile #f])
  ...
  (if (or (bvuge depth max-depth)
    ...
    (begin
      (debugln (~a "Result: " (read-register (state-regs curr-state) (bv8 0))))
      (if outfile
        (displayln (bitvector->natural depth) outfile)
        (displayln (bitvector->natural depth)))
      curr-state)
      (let* ([rvals (state-rvals curr-state)]
        ...
        (begin
          (debugln (~a "Instr: " imm " " offset " " src " " dst " " opcode))
          (debug-registers rvals DEBUG)
          (case-op opcode
            ; lddw
